...
(function($){
	var defaultSettings = {
		...
		closeButton: '/ui/shared/images/tb-close-button.gif',
		onShow: function() { }
	};
	...
	var Thinbox = function(element,options) {
		...
		this.showModal = function(element,url) {
			if(url!=null) {
				...
				var href = $(element).attr("data-url")||$(element).attr("href")||$(element).attr("alt")||'';
			}
			...
			if(href.substr(0,1)=="#") {
				...
				var imageDesc = $('<p/>').html($($(element).attr('title')).html());
				var inIframe = false;
				var isImage = true;
			} else if(href=='') {
				 return false;
	...
	$(document).ready(function(){
		ThinBox.modals = $("a[data-type='modal'],input[data-type='modal']").thinbox({
